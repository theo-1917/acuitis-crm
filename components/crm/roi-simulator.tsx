...
export function RoiSimulator() {
  ...
  for (let y = 0; y < 5; y++) {
    ...
    const personnel = calcSalaires(currentYear)
  ...
  return (
    <>
      ...
      */}
      ...
      <div id="roi-print-section" className="flex flex-col gap-6 max-h-[calc(100vh-140px)] overflow-y-auto pr-2 pb-10">
        
        {/* HEADER */}
        <div className="flex flex-wrap items-center justify-between gap-4 rounded-xl border border-border bg-card p-5 shrink-0 shadow-sm print-hide-border">
          <div className="flex items-center gap-3">
            <div className="bg-primary/10 p-2.5 rounded-lg text-primary print-hide"><Calculator className="h-6 w-6" /></div>
            <div>
              <h2 className="text-xl font-bold text-foreground">Simulateur d'Investissement & ROI <span className="hidden print:inline-block">- Acuitis</span></h2>
          ...
          <div className="flex gap-4 items-center">
            ...
            <Button onClick={handlePrint} className="ml-4 print-hide bg-indigo-600 hover:bg-indigo-700 text-white">
        ...
        <div className="grid grid-cols-1 xl:grid-cols-12 gap-6 print:block">
          ...
          <div className="xl:col-span-7 flex flex-col gap-5 print:mb-8">
            ...
            <div className="rounded-xl border border-border bg-card p-5 print:shadow-none print:border-gray-200">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-5">
                ...
                <div className="bg-muted/20 p-4 rounded-lg border border-border flex flex-col justify-center print:bg-white print:border-gray-200">
                  ...
                  <div className="flex justify-between text-[10px] text-muted-foreground mt-1 print-hide"><span>62%</span><span>66%</span><span>70%</span></div>
                </div>
              </div>
            ...
            <div className="rounded-xl border border-border bg-card p-5 print:shadow-none print:border-gray-200 print:mt-6">
              <h3 className="font-semibold text-foreground text-sm flex items-center gap-2 border-b border-border pb-3 mb-4"><Euro className="h-4 w-4 text-primary print-hide" /> 2. Investissements de départ (HT)</h3>
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                ...
                <div><label className="text-[11px] text-violet-500 font-semibold mb-1 block">Matériel Leasing (5 ans)</label><Input type="number" value={materielLeasing} onChange={e => setMaterielLeasing(Number(e.target.value))} className="text-xs border-violet-500/30 bg-violet-500/5 print:bg-white" /></div>
            ...
            <div className="rounded-xl border border-blue-500/20 bg-blue-900/5 p-5 print:bg-white print:border-gray-200 print:mt-6">
              <h3 className="font-semibold text-blue-500 text-sm flex items-center gap-2 border-b border-blue-500/20 pb-3 mb-4"><Landmark className="h-4 w-4 print-hide" /> 3. Financement Bancaire</h3>
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                ...
                <div><label className="text-[11px] text-muted-foreground mb-1 block">Franchise décalage (Mois)</label><Input type="number" value={decalageMois} onChange={e => setDecalageMois(Number(e.target.value))} className="text-xs" /></div>
              </div>
            </div>

            {/* 4. FRAIS FIXES & LOCAL */}
            <div className="rounded-xl border border-border bg-card p-5 print:shadow-none print:border-gray-200 print:mt-6">
              <h3 className="font-semibold text-foreground text-sm flex items-center gap-2 border-b border-border pb-3 mb-4"><Building className="h-4 w-4 text-primary print-hide" /> 4. Local & Frais Fixes</h3>
          ...
          <div className="xl:col-span-5 flex flex-col gap-5 print:block print:mt-12" style={{ pageBreakBefore: 'always' }}>
            ...
            <div className="rounded-xl border-2 border-emerald-500/20 bg-card shadow-lg overflow-hidden shrink-0 print:border-gray-300 print:shadow-none">
              <div className="bg-emerald-500/10 px-5 py-4 border-b border-emerald-500/20 print:bg-gray-100">
                <h3 className="font-bold text-emerald-500 flex items-center gap-2"><TrendingUp className="h-5 w-5 print-hide" /> Seuils de Rentabilité (SR)</h3>
                ...
                {[1, 2, 3].map((y) => {
                  ...
                  return (
                    <div key={y} className="bg-background p-3 rounded border border-border flex flex-col gap-2 print:border-gray-200">
                      ...
                      <div className="flex justify-between items-center text-[10px] text-muted-foreground bg-muted/30 px-2 py-1.5 rounded print:bg-gray-50">
            ...
            <div className="rounded-xl border border-border bg-card p-5 shadow-sm flex-1 flex flex-col print:shadow-none print:border-gray-200 print:mt-6">
              <h3 className="font-bold text-foreground flex items-center gap-2 border-b border-border pb-3 mb-4"><FileSpreadsheet className="h-5 w-5 text-primary print-hide" /> Synthèse Rentabilité (P&L en k€)</h3>
              ...
              <div className="overflow-x-auto pb-4 print:overflow-visible">
                <table className="w-full text-[11px] text-left">
                  ...
                  <tbody className="divide-y divide-border font-medium">
                    {/* REVENUS */}
                    <tr className="bg-muted/10 print:bg-gray-100"><td colSpan={6} className="py-1 pl-1 text-[10px] text-muted-foreground uppercase font-bold tracking-wider">Revenus</td></tr>
                    ...
                    <tr className="bg-primary/5 font-bold print:bg-blue-50"><td className="py-1.5 pl-1 text-primary">CA NET EN K€ HT</td><td className="py-1.5 text-right">{fK(yearsData[0].caTot)}</td><td className="py-1.5 text-right">{fK(yearsData[1].caTot)}</td><td className="py-1.5 text-right">{fK(yearsData[2].caTot)}</td><td className="py-1.5 text-right">{fK(yearsData[3].caTot)}</td><td className="py-1.5 text-right">{fK(yearsData[4].caTot)}</td></tr>
                    ...
                    {/* FRAIS MAGASIN */}
                    <tr className="bg-muted/10 print:bg-gray-100"><td colSpan={6} className="py-1 pl-1 text-[10px] text-muted-foreground uppercase font-bold tracking-wider">Frais Magasin</td></tr>
                    ...
                    {/* PERFORMANCE MAGASIN */}
                    <tr className="bg-emerald-500/10 font-bold print:bg-emerald-50"><td className="py-1.5 pl-1 text-emerald-600">EBITDA Magasin</td><td className="py-1.5 text-right">{fK(yearsData[0].ebitdaMagasin)}</td><td className="py-1.5 text-right">{fK(yearsData[1].ebitdaMagasin)}</td><td className="py-1.5 text-right">{fK(yearsData[2].ebitdaMagasin)}</td><td className="py-1.5 text-right">{fK(yearsData[3].ebitdaMagasin)}</td><td className="py-1.5 text-right">{fK(yearsData[4].ebitdaMagasin)}</td></tr>
                    ...
                    {/* REDEVANCES */}
                    <tr className="bg-muted/10 print:bg-gray-100"><td colSpan={6} className="py-1 pl-1 text-[10px] text-muted-foreground uppercase font-bold tracking-wider">Réseau & Franchise</td></tr>
                    ...
                    {/* PERFORMANCE STE */}
                    <tr className="bg-primary/10 font-bold border-t-2 border-border print:bg-blue-50"><td className="py-1.5 pl-1 text-primary">EBITDA après Franchise</td><td className="py-1.5 text-right">{fK(yearsData[0].ebitdaApresFranchise)}</td><td className="py-1.5 text-right">{fK(yearsData[1].ebitdaApresFranchise)}</td><td className="py-1.5 text-right">{fK(yearsData[2].ebitdaApresFranchise)}</td><td className="py-1.5 text-right">{fK(yearsData[3].ebitdaApresFranchise)}</td><td className="py-1.5 text-right">{fK(yearsData[4].ebitdaApresFranchise)}</td></tr>
                    ...
                    {/* CASH FLOW */}
                    <tr className="bg-muted/10 print:bg-gray-100"><td colSpan={6} className="py-1 pl-1 text-[10px] text-muted-foreground uppercase font-bold tracking-wider border-t border-border">Trésorerie (Cash Flow)</td></tr>
              ...
              {/* METRICS FINALES ROI */}
              <div className="bg-primary/5 rounded-lg p-4 border border-primary/20 mt-auto print:bg-blue-50 print:border-blue-200">
                ...
                <div className="space-y-3">
                  ...
                  <div className="pt-2 border-t border-primary/10 flex justify-between items-center print:border-blue-200">
                    <span className="text-xs font-bold text-foreground">R.O.I à 5 ans</span>
                    <Badge className="bg-primary hover:bg-primary text-white text-sm print:bg-transparent print:text-primary print:border print:border-primary">
